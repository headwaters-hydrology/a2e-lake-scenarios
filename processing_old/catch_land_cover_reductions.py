    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=3, mp_context=mp.get_context("spawn")) as executor:
        ...
        with booklet.open(params.sites_catch_major_path) as catches:
            for catch_id, catch in catches.items():
                ...
                f1 = executor.submit(utils.calc_reductions, catch_id, poly, lcdb0, snb_dairy0)
                futures[f1] = catch_id

        # Save results
        with booklet.open(params.sites_land_cover_reductions_path, 'n', value_serializer='gpd_zstd', key_serializer='uint4', n_buckets=1607) as f:
            counter = 0
