...
if __name__ == '__main__':
    with concurrent.futures.ProcessPoolExecutor(max_workers=10, mp_context=mp.get_context("spawn")) as executor:
        futures = {}
        with booklet.open(params.sites_catch_major_path) as catches:
            for catch_id, catch in catches.items():
                # if catch_id == 3047194:
                #     d
                poly = gpd.GeoSeries([catch], crs=4326).to_crs(2193).iloc[0]

                f1 = executor.submit(utils.calc_yields, catch_id, poly, snb_dairy0, yields0, slope_geo0, moisture_geo0, lcdb2)
                futures[f1] = catch_id

        # Save results
        with booklet.open(params.sites_yields_blt_path, 'n', value_serializer='gpd_zstd', key_serializer='uint4', n_buckets=1607) as f:
            counter = 0
