...
def process_flows_rec():
    ...
    flows = {way_id: v['Mean Flow'] for way_id, v in w0._way_tag.items()}
    ...
    # ways_up = set(list(way_index.keys()))
    # way_index_up = {way_id: set(v).intersection(ways_up) for way_id, v in way_index.items()}
    ...
    with booklet.open(params.river_flows_rec_diff_path, 'n', key_serializer='uint4', value_serializer='uint4') as f:
